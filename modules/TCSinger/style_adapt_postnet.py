...
class GaussianDiffusion(nn.Module):
    def __init__(self, phone_encoder, out_dims, denoise_fn,
        ...
        if exists(betas):
            ...
            if 'post_schedule_type' in hparams.keys():
                betas = beta_schedule[hparams['post_schedule_type']](timesteps)
            else:
    ...
    def p_mean_variance(self, x, t, cond, style, clip_denoised: bool):
        noise_pred = self.denoise_fn(x, t, cond=cond, style=style)
    ...
    def p_losses(self, x_start, t, cond, style,noise=None, nonpadding=None):
        noise = default(noise, lambda: torch.randn_like(x_start))

        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_recon = self.denoise_fn(x_noisy, t, cond,style)
...
# SAP
class StylePostnet(GaussianDiffusion):
    def forward(self, cond, ref_mels, coarse_mels, ret, infer):
        ...
        if not infer:
            ...
            x = x.transpose(1, 2)[:, None, :, :]  # [B, 1, M, T]
            ret['diff'] = self.p_losses(x, t, cond,style)
        else:
            t = self.K_step
            fs2_mels = self.norm_spec(fs2_mels)
            fs2_mels = fs2_mels.transpose(1, 2)[:, None, :, :]
            ...
            for i in tqdm(reversed(range(0, t)), desc='sample time step', total=t):
                x = self.p_sample(x, torch.full((b,), i, device=device, dtype=torch.long), cond,style)
            x = x[:, 0].transpose(1, 2)
            ret['mel_out'] = self.denorm_spec(x)
            ret['diff'] = 0.0
