...
from modules.TCSinger.diff.shallow_diffusion import *
import numpy as np
import torch
...
class SADecoder(nn.Module):
    ...
    @torch.no_grad()
    def p_sample(self, x_t, t, cond, style,spk_emb=None, clip_denoised=True, repeat_noise=False):
        b, *_, device = *x_t.shape, x_t.device
        x_0_pred = self.denoise_fn(x_t, t, cond,style)
    ...
    def forward(self, txt_tokens, ret,
        ...
        if not infer:
            ...
            # Predict x_{start}
            x_0_pred = self.denoise_fn(x_t, t, cond,style) * nonpadding
            ...
            for i in tqdm(reversed(range(0, t)), desc='ProDiff Teacher sample time step', total=t):
                x = self.p_sample(x, torch.full((b,), i,device=device, dtype=torch.long), cond,style=style)  # x(mel), t, condition(phoneme)
            x = x[:, 0].transpose(1, 2)
